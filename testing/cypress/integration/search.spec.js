const config = {
	url: 'https://www.bbwheelsonline.com/search?search_query=red',
	selectors: {
		sidebar: {
			wrapper: '#facetedSearch',
...
describe('Search Page', () => {
	...
	describe('Refinements', () => {
		it('has correct titles', function () {
			if (!config?.selectors?.sidebar?.wrapper && !config?.selectors?.sidebar?.facetTitle) this.skip();

			cy.snapStore().then((store) => {
				cy.get(config.selectors.sidebar.wrapper)
					.find(config.selectors.sidebar.facetTitle)
		...
		it('can toggle facet collapse', function () {
			if (
				!config?.selectors?.sidebar?.wrapper &&
			...
			cy.snapStore().then((store) => {
				cy.get(config.selectors.sidebar.wrapper)
					.find(config.selectors.sidebar.facetWrapper)
					.each((el, index) => {
		...
		it('can toggle facet overflow', function () {
			if (
				!config?.selectors?.sidebar?.wrapper &&
				!config?.selectors?.sidebar?.facetWrapper &&
				!config?.selectors?.sidebar?.showMoreButton &&
			...
			cy.snapStore().then((store) => {
				...
				cy.get(`${config.selectors.sidebar.wrapper} ${config.selectors.sidebar.facetWrapper}`)
					.each((el, index) => {
		...
		it('can search options within facet', function () {
			if (
				!config?.selectors?.sidebar?.wrapper &&
				!config?.selectors?.sidebar?.facetWrapper &&
				!config?.selectors?.sidebar?.searchWithinInput &&
				!config?.selectors?.sidebar?.facetOption
			)
				this.skip();
		...
		it('can select a list option', function () {
			...
			cy.snapStore().then((store) => {
				...
				listFacet &&
					cy
						.get(`${config.selectors.sidebar.wrapper} ${config.selectors.sidebar.facetWrapper}`)
						.each((facet, index) => {
						...
						.then(() => {
							...
							const facetListOption = facetListElement.find(config.selectors.sidebar.facetOption)[1]; // 1 has more values for further refinements
			...
			cy.snapStore().then((store) => {
				...
				gridFacet &&
					cy
						...
						.then(() => {
							// click on an option in facet and ensure urlManager contains new state
							const facetGridOption = facetGridElement.find(config.selectors.sidebar.facetOption)[4]; // 4 has more values for further refinements
			...
			cy.snapStore().then((store) => {
				...
				paletteFacet &&
					cy
						...
						.then(() => {
							// click on an option in facet and ensure urlManager contains new state
							const facetPaletteOption = facetPaletteElement.find(config.selectors.sidebar.facetOption)[4]; // 4 has more values for further refinements
				...
				hierarchyFacet &&
					cy
						...
						.then(() => {
							...
							const clickedValue = facetHierarchyOption.innerText;
							facetHierarchyOption.click();
							cy.snapStore().then((store) => {
								expect(Object.keys(store.controller.urlManager.state.filter)).to.contain(hierarchyFacet.field);
								const value = store.controller.urlManager.state.filter[hierarchyFacet.field][0];
								expect(clickedValue).to.contain(value);
							});
						});
			});
